...
import {
  TbCode, TbEye, TbBox, TbCopy, TbCheck, TbPackage, TbBrandNpm,
  TbChevronRight, TbSearch, TbLayoutSidebarLeftExpand, TbX, TbMenu2,
} from "react-icons/tb";
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 sm:px-8 text-center py-10 sm:py-16">
      ...
      >
        ...
        {userData ? (
          <>
            <h2 className="text-base sm:text-lg font-gilroy font-bold mb-2 text-white/80">
              Select a component
            </h2>
            <p className="text-white/35 text-xs font-gilroy sm:text-sm mb-8 sm:mb-10 max-w-sm mx-auto leading-relaxed">
        ...
        <div className="w-full max-w-md mx-auto text-left space-y-4 mb-8">
          <p className="text-[10px] font-gilroy font-bold tracking-[3px] uppercase text-[#3be8ff]/50 mb-4">
            Quick Start Guide
          </p>

          <div>
            <p className="text-xs font-gilroy text-white/40 mb-2 flex items-center gap-1.5">
              <span className="text-[#3be8ff]/60 font-gilroy font-bold">01</span>
              Install the package
            </p>
            <CodeBlock code={`npm install "@prateekrwt07/virtualui"`} lang="bash" />
          </div>

          <div>
            <p className="text-xs font-gilroy text-white/40 mb-2 flex items-center gap-1.5">
              <span className="text-[#3be8ff]/60 font-gilroy font-bold">02</span>
              Import your component
            </p>
            <CodeBlock
              code={`import {componentName} from "@prateekrwt07/virtualui";`}
              lang="jsx"
            />
          </div>
          ...
          <div>
            <p className="text-xs font-gilroy text-white/40 mb-2 flex items-center gap-1.5">
              <span className="text-[#3be8ff]/60 font-gilroy font-bold">03</span>
              Use in your App.jsx
            </p>
            <CodeBlock
        ...
        <p className="text-white/20 font-gilroy text-xs">
...
function DetailPanel({ component, onBack }) {
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.06] gap-3">
        {/* Left: back btn + title */}
        <div className="flex items-center gap-2.5 min-w-0">
          <button
            onClick={onBack}
            className="sm:hidden flex items-center justify-center w-7 h-7 rounded-lg bg-white/[0.05] border border-white/[0.08] text-white/50 hover:text-white/80 transition-colors cursor-pointer shrink-0"
          >
            <TbX size={13} />
          </button>

          <div className="min-w-0">
            <h2 className="text-sm font-gilroy font-bold text-white truncate leading-tight">
              {component.name}
            </h2>
            <p className="text-white/30 font-gilroy text-[10px] mt-0.5 truncate">
        ...
        {/* Right: tab switcher */}
        <div
          className="flex gap-0.5 rounded-xl p-1 shrink-0"
        ...
        >
          {tabs.map(({ id, icon: Icon, label }) => (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-gilroy font-medium transition-all capitalize cursor-pointer border-none whitespace-nowrap"
              style={{
                background: activeTab === id ? "rgba(59,232,255,0.15)" : "transparent",
                color: activeTab === id ? "#3be8ff" : "rgba(255,255,255,0.5)",
              }}
            >
              <Icon size={11} />
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "guide" && (
            ...
            >
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-white/[0.05] bg-white/[0.02]">
                          <th className="text-left px-4 py-2.5 font-gilroy text-white/35 font-medium">Name</th>
                          <th className="text-left px-4 py-2.5 font-gilroy text-white/35 font-medium">Type</th>
                        </tr>
                      </thead>
                      <tbody>
                        {component.props.map((p, i) => (
                          <tr key={i} className="border-b border-white/[0.04] last:border-0">
                            ...
                            <td className="px-4 py-2.5 font-gilroy text-white/20">any</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div>
                <p className="text-xs font-semibold font-gilroy text-white/50 mb-3 flex items-center gap-2">
    ...
    <>
      <div className="p-3 border-b border-white/[0.05]">
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06]">
          ...
          <input
            ...
            value={search}
            className="bg-transparent text-xs font-gilroy text-white/70 placeholder-white/20 outline-none w-full"
          />
        </div>
      </div>

      <div className="px-4 pt-3 pb-1.5">
        <p className="text-[9px] font-bold font-gilroy tracking-[2.5px] uppercase text-white/20">
      ...
      <div className="flex-1 overflow-y-auto py-1 px-2">
        {publicComponents.length === 0 ? (
          <p className="text-white/20 font-gilroy text-xs text-center py-8 px-3">
          ...
          publicComponents.map((c) => (
            <button
              key={c._id}
              onClick={() => onSelect(c)}
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-gilroy transition-all cursor-pointer border text-left mb-0.5"
              style={{
            ...
            >
              <span className="truncate font-gilroy font-medium text-xs">{c.name}</span>
  ...
  return (
    ...
    >
      ...
      <nav className="sticky top-0 z-40 flex items-center justify-between px-4 sm:px-8 py-3.5 sm:py-4 border-b border-white/[0.05] bg-[#030b0d]/90 backdrop-blur-md shrink-0">
        ...
        >
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-br from-[#3be8ff] to-[#04b5d4] flex items-center justify-center shadow-[0_0_14px_rgba(59,232,255,0.35)]">
            <SiValorant size={13} color="#051c20" />
          </div>
          <span className="text-[16px] sm:text-[17px] font-normal text-white font-gilroy">
            VirtualUI
          </span>
        </button>

        <div className="flex items-center gap-2">
          <div className="hidden font-gilroy sm:flex items-center gap-2 text-[13px] text-white/30">
            <TbLayoutSidebarLeftExpand size={17} />
            <span>Component Explorer</span>
          </div>
          <button
            onClick={() => setSidebarOpen(true)}
            className="sm:hidden flex items-center justify-center w-8 h-8 rounded-xl font-gilroy bg-white/[0.04] border border-white/[0.08] text-white/50 hover:text-white/80 transition-colors cursor-pointer"
          >
            <TbMenu2 size={16} />
          </button>
        </div>
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between px-4 py-4 border-b border-white/[0.06]">
                  <span className="text-xs font-gilroy font-bold text-white/40 tracking-widest uppercase">
                    Components
                  </span>
                  <button
                    onClick={() => setSidebarOpen(false)}
